/**
 * ═══════════════════════════════════════════════════════════════════════════════
 * SCRAPE ROUTES - Alexandria Service
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * Web scraping, content archiving, and media processing.
 * Uses oipClient to communicate with oip-daemon-service for data operations.
 * 
 * Publishing operations are proxied through oipClient to daemon endpoints:
 *   - /api/publish/newPost
 *   - /api/publish/newNutritionalInfo
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
// Note: These templateHelper functions are only used for media processing, not direct publishing
const { uploadToArFleet, publishVideoFiles, publishArticleText, publishImage } = require('../../helpers/core/templateHelper');
const { authenticateToken, getWalletFilePath } = require('../../helpers/utils'); // Import the authentication middleware
const { ongoingScrapes, cleanupScrape } = require('../../helpers/core/sharedState.js'); // Adjust the path to store.js
...
async function getRecords(params, req = null) {
    const oipClient = req ? getOIPClient(req) : defaultOIPClient;
    return oipClient.getRecords(params);
}

/**
 * Publish a post record via oipClient to daemon
 * @param {object} recordData - The post record data
 ...
 * @returns {Promise<object>} Published record result
 */
async function publishPost(recordData, blockchain = 'arweave', oipClient = null) {
    const client = oipClient || defaultOIPClient;
    
    try {
        console.log('[Scrape] Publishing post via oipClient to daemon...');
        const result = await client.request('POST', '/api/publish/newPost', {
            ...recordData,
            blockchain
        });
        
        // Map response to match expected format from publishNewRecord
        return {
            transactionId: result.transactionId,
            recordToIndex: result.recordToIndex,
            did: result.recordToIndex?.oip?.did || result.recordToIndex?.oip?.didTx,
            didTx: result.recordToIndex?.oip?.didTx
        };
    } catch (error) {
        console.error('[Scrape] Error publishing post via oipClient:', error.message);
        throw error;
    }
}

/**
 * Publish a nutritional info record via oipClient to daemon
 * @param {object} nutritionalData - The nutritional info data
 * @param {string} blockchain - Blockchain to publish to (default: 'arweave')
 * @param {OIPClient} oipClient - Optional custom client instance
 * @returns {Promise<object>} Published record result
 */
async function publishNutritionalInfo(nutritionalData, blockchain = 'arweave', oipClient = null) {
    const client = oipClient || defaultOIPClient;
    
    try {
        console.log('[Scrape] Publishing nutritional info via oipClient to daemon...');
        const result = await client.request('POST', '/api/publish/newNutritionalInfo', {
            ...nutritionalData,
            blockchain
        });
        ...
        console.error('[Scrape] Error publishing nutritional info via oipClient:', error.message);
        throw error;
    }
}
...
const {
  ...
  synthesizeSpeech
} = require('../../helpers/core/generators');
const {getCurrentBlockHeight, getBlockHeightFromTxId, lazyFunding, upfrontFunding, arweave} = require('../../helpers/core/arweave');
...
const { getBaseUrl, getMediaUrl } = require('../../helpers/core/urlHelper');
  ...
  try {
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      messages: [
              {
                role: 'system',
                content: 'You are a recipe extraction expert. Extract complete recipe information from the provided text. Parse ingredient strings to separate amounts, units, names, and comments. CRITICAL: For "COUNT (SIZE) ITEM" patterns like "4 (4-ounce) salmon fillets", MULTIPLY to get the TOTAL in standard units: amount=16, unit="ounces", comment="4 fillets × 4 ounces each". The total amount and standard unit are needed for nutritional calculations. Comments also include preparation notes like "minced", "chopped", "sliced", "diced", "optional", "to taste", etc. For timing, convert to minutes. Identify the recipe author if mentioned. Intelligently determine the cuisine type and course/meal type based on the recipe content. CRITICAL FOR TITLES: Recipe titles MUST be NOUN PHRASES, never imperative commands or action statements. If the title starts with a verb (Make, Try, Cook, etc.), remove the entire imperative clause and extract ONLY the noun phrase that names the dish. Example: "Make This Super-Simple Salad in About 5 Minutes!" → "Simple 5-Minute Salad" (removed "Make This" and "in About", simplified "Super-Simple" to "Simple").'
              },
              {
                role: 'user',
                content: `Extract the recipe from this webpage text. Return complete recipe data including title, description, author, ingredients (with amount, unit, name, and comment separated), instructions, timing, servings, cuisine, and course.\n\n🔴 CRITICAL TITLE RULE: The title field MUST contain ONLY a NOUN PHRASE - the name of the dish itself. NEVER include imperative verbs, action commands, or promotional language.\n\n✂️ TITLE PROCESSING STEPS:\n1. If the title starts with a VERB (Make, Try, Cook, Bake, Prepare, etc.), DELETE the entire verb phrase\n2. Remove ALL demonstratives: "This", "These", "That", "Those"\n3. Remove promotional adjectives: "Amazing", "Perfect", "Ultimate", "Delicious"\n4. Remove temporal/urgency phrases: "in About", "in Just", "Tonight", "Right Now"\n5. Remove personal appeals: "You'll Love", "I Make", "My Favorite"\n6. Simplify redundant adjectives: "Super-Simple" → "Simple", "Very Easy" → "Easy"\n7. Remove exclamation marks and trailing "Recipe" or "Dish"\n8. Keep timing/count descriptors: "5-Minute", "3-Ingredient", "One-Pan"\n9. Keep cooking methods: "Grilled", "Roasted", "Baked"\n\n📋 TITLE TRANSFORMATION EXAMPLES (MUST FOLLOW THIS PATTERN):\n\n❌ "Make This Super-Simple Salad in About 5 Minutes!"\n   Step 1: Remove "Make" → "This Super-Simple Salad in About 5 Minutes!"\n   Step 2: Remove "This" → "Super-Simple Salad in About 5 Minutes!"\n   Step 3: Simplify "Super-Simple" → "Simple Salad in About 5 Minutes!"\n   Step 4: Convert "in About 5 Minutes" to descriptor → "Simple 5-Minute Salad"\n✅ RESULT: "Simple 5-Minute Salad"\n\n❌ "Try Our Amazing Quick and Easy Chocolate Chip Cookies Tonight!"\n   Remove "Try Our" → Remove "Amazing" → Remove "Tonight" → Simplify "Quick and Easy" → Keep "Chocolate Chip Cookies"\n✅ RESULT: "Chocolate Chip Cookies"\n\n❌ "Spice Up Your Salsa Repertoire With This Tomatillo Avocado Salsa"\n   Remove "Spice Up Your Salsa Repertoire With" → Remove "This"\n✅ RESULT: "Tomatillo Avocado Salsa"\n\n❌ "The 5-Ingredient, 15-Minute Tortilla Soup I Make At Least Twice a Month"\n   Keep count/time descriptors → Remove "I Make At Least Twice a Month"\n✅ RESULT: "5-Ingredient 15-Minute Tortilla Soup"\n\n❌ "You'll Love This One-Pan Creamy Chicken Pasta Recipe"\n   Remove "You'll Love" → Remove "This" → Keep "One-Pan" → Remove "Recipe"\n✅ RESULT: "One-Pan Creamy Chicken Pasta"\n\n❌ "The Best Grilled Cheese Sandwich You'll Ever Make"\n   Remove "The Best" → Keep "Grilled" (cooking method) → Remove "You'll Ever Make"\n✅ RESULT: "Grilled Cheese Sandwich"\n\n❌ "Prepare This Delicious Homemade Tomato Soup in Just 20 Minutes"\n   Remove "Prepare" → Remove "This" → Remove "Delicious" → Convert time to descriptor\n✅ RESULT: "20-Minute Tomato Soup"\n\n🎯 FINAL CHECK: Does the title start with a noun or adjective? YES = correct. Starts with a verb? NO = wrong, fix it.\n\nFor ingredients, MULTIPLY counts by sizes to get TOTAL AMOUNTS in standard units (needed for nutritional calculations).

CRITICAL - "COUNT (SIZE) ITEM" pattern - MULTIPLY to get total:
- "4 (4-ounce) salmon fillets, skin-on" → amount: 16, unit: "ounces", name: "salmon fillets", comment: "4 fillets × 4 ounces each, skin-on"
- "4 (4- to 5-ounce) salmon fillets" → amount: 18, unit: "ounces", name: "salmon fillets", comment: "4 fillets × 4.5 ounces each (average)"
- "2 (15-ounce) cans beans" → amount: 30, unit: "ounces", name: "beans", comment: "2 cans × 15 ounces each"
- "3 (6-inch) tortillas" → amount: 3, unit: "", name: "tortillas", comment: "6-inch diameter"

Standard examples (already in usable units):
...
async function publishArticleAndAttachedMedia(articleData, $, url, html, res, blockchain = 'arweave') {
  ...
  // Publish the main record via oipClient to daemon
  const record = await publishPost(recordToPublish, blockchain);
...
async function createNewNutritionalInfoRecord(ingredientName, blockchain = 'arweave') {
  try {
    ...
    // Publish nutritional info via oipClient to daemon
    ingredientTx = await publishNutritionalInfo(formattedNutritionalInfo, blockchain);

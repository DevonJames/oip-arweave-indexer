  ...
  try {
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      messages: [
              {
                role: 'system',
                content: 'You are a recipe extraction expert. Extract complete recipe information from the provided text. Parse ingredient strings to separate amounts, units, names, and comments. CRITICAL: For "COUNT (SIZE) ITEM" patterns like "4 (4-ounce) salmon fillets", MULTIPLY to get the TOTAL in standard units: amount=16, unit="ounces", comment="4 fillets × 4 ounces each". The total amount and standard unit are needed for nutritional calculations. Comments also include preparation notes like "minced", "chopped", "sliced", "diced", "optional", "to taste", etc. For timing, convert to minutes. Identify the recipe author if mentioned. Intelligently determine the cuisine type and course/meal type based on the recipe content. Extract clean recipe titles, not article headlines - remove promotional phrases like "Spice Up Your...", "Try This...", "The Best...", "You\'ll Love...", etc.'
              },
              {
                role: 'user',
                content: `Extract the recipe from this webpage text. Return complete recipe data including title, description, author, ingredients (with amount, unit, name, and comment separated), instructions, timing, servings, cuisine, and course.\n\nFor the title, extract ONLY the recipe name, not article headlines. Remove ALL promotional/editorial/imperative phrases:\n\nREMOVE these patterns:\n- Imperative verbs: "Make", "Try", "Cook", "Bake", "Prepare", "Create", "Discover", "Learn", etc.\n- Demonstratives: "This", "These", "That", "Those" (when not part of recipe name)\n- Promotional: "The Best", "Perfect", "Ultimate", "Amazing", "Delicious", "Easy", "Quick", "Simple" (unless describing cooking method)\n- Temporal phrases: "in About", "in Just", "in Only", "in Minutes", "Tonight", "Today"\n- Personal phrases: "I Make", "You'll Love", "My Favorite", "Our Family's"\n- Exclamation marks and extra punctuation\n- Recipe/dish type suffixes: "Recipe", "Dish" (unless part of actual name like "Rice Dish")\n\nKEEP useful descriptors:\n- Cooking times when they're part of the name: "5-Minute Salad", "30-Minute Pasta"\n- Ingredient counts: "5-Ingredient Soup", "Three-Cheese Pizza"\n- Cooking methods: "Grilled", "Roasted", "Pan-Seared", "Slow-Cooked"\n- Key descriptors: "Super-Simple" → "Simple", "One-Pan" → "One-Pan"\n\nTitle examples:\n- ❌ "Make This Super-Simple Salad in About 5 Minutes!" → ✅ "Simple 5-Minute Salad"\n- ❌ "Spice Up Your Salsa Repertoire With This Tomatillo Avocado Salsa" → ✅ "Tomatillo Avocado Salsa"\n- ❌ "The 5-Ingredient, 15-Minute Tortilla Soup I Make At Least Twice a Month" → ✅ "5-Ingredient Tortilla Soup"\n- ❌ "You'll Love This One-Pan Creamy Chicken Pasta Recipe" → ✅ "One-Pan Creamy Chicken Pasta"\n- ❌ "Try Our Amazing Quick and Easy Chocolate Chip Cookies Tonight!" → ✅ "Quick Chocolate Chip Cookies"\n- ❌ "The Best Grilled Cheese Sandwich You'll Ever Make" → ✅ "Grilled Cheese Sandwich"\n- ❌ "Prepare This Delicious Homemade Tomato Soup in Just 20 Minutes" → ✅ "20-Minute Tomato Soup"\n\nFor ingredients, MULTIPLY counts by sizes to get TOTAL AMOUNTS in standard units (needed for nutritional calculations).

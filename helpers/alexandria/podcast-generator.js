/**
 * ═══════════════════════════════════════════════════════════════════════════════
 * PODCAST GENERATOR - Alexandria Service Helper
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * Generates podcasts and investigative reports from articles.
 * Uses oipClient for data operations (indexing, creator lookup).
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 */
const fs = require('fs');
...
const {synthesizeSpeech} = require('../core/generators');
const { isInt16Array } = require('util/types');
const { publishNewRecord } = require('../core/templateHelper');
const { host } = require('../../config/arweave.config');
const {getCurrentBlockHeight} = require('../core/arweave');
...
/**
 * Index a record via oipClient to daemon
 * @param {object} record - The record to index
 * @returns {Promise<void>}
 */
async function indexRecord(record) {
    try {
        await defaultOIPClient.indexRecord(record);
    } catch (error) {
        console.error('[Podcast Generator] Error indexing record via oipClient:', error.message);
        throw error;
    }
}

/**
 * Search for a creator by DID address via oipClient
 * @param {string} didAddress - The creator's DID address
 * @returns {Promise<object>} Creator data
 */
async function searchCreatorByAddress(didAddress) {
    try {
        const result = await defaultOIPClient.getCreatorByAddress(didAddress);
        return result.creator;
    } catch (error) {
        console.error('[Podcast Generator] Error searching for creator via oipClient:', error.message);
        throw error;
    }
}

/**
 * Get records via oipClient
...
async function generatePodcastFromArticles(articles, selectedHosts, targetLengthSeconds = 1500, podcastId, res) {
  ...
  try {
    ...
    //     // ],
      ...
      "audio": {
            "webUrl": require('../core/urlHelper').getMediaUrl(podcastFile),

    ...
    const processValue = (obj, fieldName = '', recordTypeFields = null) => {
        ...
        if (Array.isArray(obj)) {
            ...
            if (expectedType && expectsString(expectedType)) {
                console.log(`      🔧 [processRecord] Template expects string for '${fieldName}', stringifying array`);
        ...
        if (typeof obj === 'object') {
            ...
            if (expectedType && expectsString(expectedType)) {
                console.log(`      🔧 [processRecord] Template expects string for '${fieldName}', stringifying object: ${JSON.stringify(obj).substring(0, 50)}...`);
                return JSON.stringify(obj);
            }
...
async function getTemplateFieldsForRecordType(recordType) {
    ...
    try {
        ...
        console.warn(`      ⚠️ [processRecord] Failed to get template fields for ${recordType}: ${e.message}`);
        templateFieldsCache.set(recordType, null);
        return null;
    }
}
...
const indexRecord = async (record) => {
    ...
    try {
        ...
        if (typeForIndex) {
            ...
            if (templateFields) {
                console.log(`      📋 [indexRecord] Got template fields for '${typeForIndex}': ${Object.keys(templateFields).length} fields`);
            }
        }

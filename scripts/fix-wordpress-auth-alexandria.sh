...
echo "═══════════════════════════════════════════════════════════════"
echo "  FIXING WORDPRESS AUTHENTICATION FOR ALEXANDRIA NODE"
echo "═══════════════════════════════════════════════════════════════"
echo ""

WORDPRESS_CONTAINER="alexandria-wordpress-1"
DAEMON_CONTAINER="alexandria-oip-daemon-service-1"

# Create mu-plugins directory
docker exec -it $WORDPRESS_CONTAINER sh -c 'mkdir -p /var/www/html/wp-content/mu-plugins'

# Create improved mu-plugin that enables Application Passwords over HTTP
docker exec -it $WORDPRESS_CONTAINER sh -c 'cat > /var/www/html/wp-content/mu-plugins/op-rest-api-auth-fix.php << "EOFPHP"
<?php
/**
...
echo ""
echo "The mu-plugin will:"
echo "  1. Enable Application Passwords over HTTP (WordPress normally disables them)"
echo "  2. Handle Basic Auth properly"
echo "  3. Strip spaces from Application Passwords (WordPress expects them without spaces for Basic Auth)"
echo "  4. Fall back to regular password authentication if Application Password fails"
echo ""
echo "This fix persists across WordPress restarts."
echo ""
echo "To test authentication, run:"
echo "  docker exec -it $DAEMON_CONTAINER node scripts/test-wordpress-auth.js"
echo ""

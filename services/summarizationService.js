...
class SummarizationService {
    ...
    async summarize(options) {
        try {
            ...
            console.log('📝 [Summarization] Generating summary for', note_type, 'note with model:', model);
            ...
            if (model === 'parallel') {
                ...
                summary = await this._callLLMParallel(prompt);
            } else {
                // Use specific model
                summary = await this._callLLMDirect(prompt, model);
            }
...
JSON RESPONSE:`;
     ...
     * @private
     */
    async _callLLMDirect(prompt, model) {
     ...
     * @private
     */
    async _callLLMParallel(prompt) {
        const axios = require('axios');
        const startTime = Date.now();
        
        console.log('🏁 [Summarization] Racing parallel LLM requests...');
        
        const requests = [];
        
        // OpenAI request
        if (process.env.OPENAI_API_KEY) {
            requests.push(this._callOpenAI(prompt, 'gpt-4o-mini'));
        }
        
        // Grok request  
        if (process.env.XAI_API_KEY) {
            requests.push(this._callGrok(prompt, 'grok-beta'));
        }
        
        // Ollama requests
        requests.push(this._callOllama(prompt, 'mistral:latest'));
        const defaultModel = process.env.DEFAULT_LLM_MODEL || 'llama3.2:3b';
        requests.push(this._callOllama(prompt, defaultModel));
     ...
     * @private
     */
    async _callOpenAI(prompt, modelName) {
        try {
            const axios = require('axios');
     ...
     * @private
     */
    async _callGrok(prompt, modelName) {
        try {
            const axios = require('axios');
     ...
     * @private
     */
    async _callOllama(prompt, modelName) {
        try {
            const axios = require('axios');
